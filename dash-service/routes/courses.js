...
router.get('/api/dash/courses/:id/comments', requireDashAuth, async (req, res) => {
  const { data: comments, error } = await mainDb.from('course_lesson_comments')
    .select('*, course_lessons(title)').eq('course_id', req.params.id).order('created_at');
  if (error) return res.status(500).json({ error: error.message });
  res.json({
...
router.post('/api/dash/courses/:id/comments', requireDashAuth, async (req, res) => {
  ...
  const { data, error } = await mainDb.from('course_lesson_comments').insert({
    course_id: req.params.id, lesson_id: lessonId, parent_id: parentId || null,
    email: 'lektor', is_instructor: true, body: body.trim()
  }).select().single();
  if (error) return res.status(500).json({ error: error.message });
